# 👋 Bonjour, je suis Maguette Sow

🎓 Étudiante en **Master 1 Ingénierie Logicielle** à l’Université Numérique Cheikh Hamidou Kane, après avoir obtenu ma **Licence en Informatique, Développement Web Mobile**.  
💻 Passionnée par le **développement web & logiciel**, je construis des projets académiques et personnels pour progresser et mettre en pratique mes compétences.  

---

## 🚀 Compétences techniques
![Java](https://img.shields.io/badge/Code-Java-orange?logo=java&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Framework-Spring%20Boot-brightgreen?logo=springboot)
![PHP](https://img.shields.io/badge/Code-PHP-blue?logo=php)
![Laravel](https://img.shields.io/badge/Framework-Laravel-red?logo=laravel)
![JavaScript](https://img.shields.io/badge/Code-JS-yellow?logo=javascript)
![MySQL](https://img.shields.io/badge/DB-MySQL-blue?logo=mysql)
![Git](https://img.shields.io/badge/Tools-Git-black?logo=git)

- **Langages** : PHP, Java, JavaScript, SQL  
- **Frameworks** : Laravel, Spring Boot  
- **CMS** : WordPress  
- **Base de données** : MySQL  
- **Outils** : Git, GitHub, VS Code  

---

## 📌 Projets notables
- [🌐 Ndimballocataire](https://dev-ndimballocataire.pantheonsite.io/)  
  Plateforme web de location développée avec **WordPress**.  

- [💸 Application de transfert d’argent](https://github.com/maguette-sow/gestion) *(en cours)*  
  Backend en **Spring Boot** + Frontend en **Angular**.  

- [💼 Mon Portfolio](https://maguette-sow.github.io/portfolio)  
  Site personnel présentant mes projets, mon CV et mes compétences.  

---

## 📫 Me contacter
- ✉️ Email : [maguettesow066@gmail.com](mailto:maguettesow066@gmail.com)  
- 📞 Téléphone : **+221 78 593 21 24**  
- 💼 LinkedIn : [linkedin.com/in/maguette-sow-05244128a](https://www.linkedin.com/in/maguette-sow-05244128a/)  
- 🐙 GitHub : [github.com/maguette-sow](https://github.com/maguette-sow)  

---

✨ Mon objectif : **devenir une développeuse polyvalente**, capable de contribuer à des projets innovants en développement web et logiciel.
